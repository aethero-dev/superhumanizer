...
**Optional — real slash commands.** To get `/humanize`, `/write-for-me`, `/ai-check`, `/new-tov`,
`/list-tov` etc. as native commands (shown in the `/` help menu), install the wrappers from `commands/`:

```bash
ln -s ~/.claude/skills/superhumanizer/commands/*.md ~/.claude/commands/
```

Without this, just trigger the skill in natural language ("humanize this…", "write me…") — both work.
